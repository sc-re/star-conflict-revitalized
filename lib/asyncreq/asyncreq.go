...
func HandleAsyncReq(hdr *protocol.Header, body []byte, seq uint16, session *session.Session) {
	...
	case types.AC_CLAN_REQUEST_PROFILE:
		handle_ac_clan_request_profile(body, seq, hdr.Sequence, session)
	...
	case types.AC_USER_PROFILE_GET:
		handle_ac_user_profile_get(body, seq, hdr.Sequence, session)
	...
	case types.AC_BUY_ITEM:
		handle_ac_buy_item(body, seq, hdr.Sequence, session)
	default:
